...
SELECT frogs.frog_id
FROM frogs
LEFT JOIN diet_asv_reads
ON frogs.frog_id = diet_asv_reads.frog_id
WHERE diet_asv_reads.frog_id IS NULL;

Result: 5 frogs (IDs 8, 10, 14, 19, 32).
-->
...
SELECT frogs.sex, AVG(morphometrics.weight_g) AS avg_weight
FROM frogs
JOIN morphometrics ON frogs.frog_id = morphometrics.frog_id
GROUP BY frogs.sex;

Result: female ~17.25 g, male ~16.65 g.
-->
...
SELECT tags.tag_model, COUNT(*) AS n_deployments
FROM deployments
JOIN tags ON deployments.tag_id = tags.tag_id
GROUP BY tags.tag_model
ORDER BY n_deployments DESC;

Result: e-obs 2g (6 deployments), ahead of ATS G5 (3) and Lotek PinPoint 75 (1).
-->
...
SELECT DISTINCT diet_asv_reads.frog_id
FROM diet_asv_reads
JOIN diet_asvs ON diet_asv_reads.asv_id = diet_asvs.asv_id
WHERE diet_asvs.family = 'Chironomidae';

Result: 13 frogs (IDs 2, 5, 9, 11, 25, 27, 28, 31, 34, 35, 36, 38, 40).
-->
...
SELECT capture_sites.site_name, AVG(morphometrics.weight_g) AS avg_weight
FROM captures
JOIN capture_sites ON captures.site_id = capture_sites.site_id
JOIN morphometrics ON captures.frog_id = morphometrics.frog_id
GROUP BY capture_sites.site_name
ORDER BY avg_weight DESC;

Result: Cattail Marsh highest (~17.40 g), then Beaver Creek (~16.95 g),
then Alder Pond (~16.91 g) — differences are fairly small.
-->
